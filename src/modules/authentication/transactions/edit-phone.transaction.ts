...
@Injectable()
export class VerifyPhoneTransaction extends BaseTransaction<
  VerifyOtpRequest,
  User
> {
  constructor(
  ...
  protected async execute(
    ...
    try {
        ...
        const number_exist=await context.findOne(User,{where:[{username:req.username,}]})

        if(number_exist)
        throw new  BadRequestException('validation.phone_exists');
